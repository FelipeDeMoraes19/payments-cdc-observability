...
| Data arrives late | `make chaos-late` restarts the generator emitting events dated 48h in the past; ingestion volume is unchanged, so the heartbeat stays quiet | `dbt source freshness` on `created_at`, the business event time, not the moment of ingestion — under `ingested_at` a pipeline reprocessing a dead source would look fresh | **Yes** — measured at the production threshold: `ERROR STALE` with the injection, `PASS` without it |
| A day of exchange rates is missing | `make chaos-fx-gap` deletes a month of quotes from bronze | `transform/dbt/tests/fx_covers_the_payments.sql`, which catches both a hole between quotes and quotes that stop short of the payments | **Yes** — measured: `FAIL 1` after injection, `PASS` after `make fx` restores them |
| PII reaches the gold layer | `make chaos-pii` rebuilds silver with masking off, shouting three times before it writes | `transform/dbt/tests/no_personal_data_in_gold.sql`, which asserts every CPF and e-mail is a hex digest — masked values look nothing like the thing they replace, which is what makes leaked ones visible | **Yes** — measured: `FAIL 56` with masking off, `PASS` with it on |
...
- [What went wrong while building this](docs/what-went-wrong.md) — three defects that were
  the same defect, and a fourth entry about a causal mechanism that was written into an ADR
  as a measured fact, stood for a day, and turned out not to reproduce
